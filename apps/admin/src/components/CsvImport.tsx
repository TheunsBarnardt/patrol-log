...
interface CsvExportButtonProps {
  ...
  label?: string;
  title?: string;
  /** When set, replaces the default download of `rows` (e.g. fetch-all then export). */
  onClick?: () => void;
}

/** Download the current (usually search-filtered) rows as CSV. */
export function CsvExportButton({
  ...
  const defaultTitle =
    rows.length === 0 && !onClick ? "Nothing to export" : `Export ${rows.length} row${rows.length === 1 ? "" : "s"}`;
  return (
    <button
      type="button"
      disabled={disabled || loading || (!onClick && rows.length === 0)}
      onClick={() => (onClick ? onClick() : downloadCsv(filename, headers, rows))}
...
interface CsvImportButtonProps {
  ...
  /** When provided, renders Export CSV in the same toolbar as Import / Template */
  exportCsv?: CsvExportSpec;
}

/** Import button + optional export + template download. */
export function CsvImportButton({ onFile, loading, label = "Import CSV", template, exportCsv }: CsvImportButtonProps) {
  ...
  function handleExport() {
    if (!exportCsv) return;
    if (exportCsv.onClick) {
      exportCsv.onClick();
      return;
    }
    if (!exportCsv.rows.length) return;
    downloadCsv(exportCsv.filename, exportCsv.headers, exportCsv.rows);
  }

  const exportDisabled =
    !!exportCsv &&
    (exportCsv.loading || (!exportCsv.onClick && exportCsv.rows.length === 0));
    ...
    <div className="flex flex-wrap items-center gap-1">
      {exportCsv && (
        <button
          ...
          title={
            exportCsv.rows.length === 0 && !exportCsv.onClick
              ? "Nothing to export"
              : `Export ${exportCsv.rows.length} row${exportCsv.rows.length === 1 ? "" : "s"}`
          }
          className="px-4 py-2 rounded-lg text-sm font-semibold border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors disabled:opacity-50 flex items-center gap-1.5"
        >
          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {exportCsv.loading ? "Exporting…" : "Export CSV"}
        </button>
      )}
      <input
        ref={inputRef}
